...
<!--		<xsl:if test="string-length(date:hour-in-day())=1">
	...
	<xsl:param name="copyright">
        ...
        <br/>
		<xsl:value-of disable-output-escaping="no" select="$config/license"/>
		<br/>
		...
		<xsl:if test="$config/feedback[@show='true']">
			<div class="feedbackLink">
				<center>
					...
					-->
					<xsl:if test="$config/feedback[@type='email']">
						<a href="http://code.google.com/p/lowra/issues/entry" target="_blank">
							<xsl:value-of select="$config/feedback/feedbackEmail/label/." />
						</a>
					</xsl:if>
			...
			<xsl:if test="$config/options[@docversion='3']">
				...
				<table cellpadding="0" cellspacing="0" border="0">
					<tr>
						...
						<td>
							<xsl:text>Flash Player 9.0</xsl:text>
						</td>
					</tr>
					<tr>
						<td style="white-space:nowrap" valign="top">
						...
						<td>
							<xsl:text>ActionScript 3.0</xsl:text>
						</td>
					</tr>
				</table>
				<p></p>
