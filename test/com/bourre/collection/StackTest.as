package com.bourre.collection
{
	...
	public class StackTest extends TestCase
	{
		...
		public function testRemove() : void
		{
			...
			assertTrue(_oS + ".remove() failed to remove " + obj, _oS.remove(obj));
			assertEquals(_oS + ".size() failed to return 5 after remove [1]", 5, _oS.size() );
			assertTrue(_oS + ".remove() failed to remove " + obj + " a second time", _oS.remove(obj));			assertFalse(_oS + ".remove() failed when attempting to removing an non existing object", _oS.remove(obj));
			assertEquals(_oS + ".size() failed to return 4 after remove [2]", 4, _oS.size());
			assertTrue(_oS + ".remove() failed to remove " + obj, _oS.remove(42));
			assertEquals(_oS + ".size() failed to return 3 after remove", 3, _oS.size());
		}
